...
int main () {
    ...
    win = XCreateWindow(display, RootWindow(display, 0), x, y, w, h, b, CopyFromParent, CopyFromParent, CopyFromParent, 0, NULL);

    a.cursor = XCreateFontCursor(display, XC_watch);
    child = XCreateWindow(display, win, x + w / 8, y + h / 8, 3 * w / 4, 3 * h / 4, 0, CopyFromParent, InputOnly, CopyFromParent, CWCursor, &a);
