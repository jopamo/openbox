...
void test_request(Display *display, Window win, Atom _request, Atom _extents, Atom _type, Atom _state, Atom state_value, const char* state_name) {
    printf("requesting for type %s\n", state_name);
    XChangeProperty(display, win, _type, XA_ATOM, 32,
                    PropModeReplace, (unsigned char*)&_type, 1);
    XChangeProperty(display, win, _state, XA_ATOM, 32,
                    PropModeReplace, (unsigned char*)&state_value, 1);
