...
RsvgLoader* LoadWithRsvg(gchar* path, RrPixel32** pixel_data, gint* width, gint* height) {
  ...
  if (!stream) {
    g_warning("Could not open file '%s': %s", path, error ? error->message : "unknown error");
    if (error) {
      g_clear_error(&error);
    }
    DestroyRsvgLoader(loader);
  ...
  if (!rsvg_handle_read_stream_sync(loader->handle, stream, NULL, &error)) {
    g_warning("Error reading SVG from '%s': %s", path, error ? error->message : "unknown error");
    if (error) {
      g_clear_error(&error);
    }
  ...
  *height = (gint)ceil(svg_h);
  ...
  // Render the full SVG document into that surface
  {
    ...
    if (!rsvg_handle_render_document(loader->handle, cr, &viewport, &error)) {
      g_warning("Error rendering SVG '%s': %s", path, error ? error->message : "unknown error");
      if (error) {
        g_clear_error(&error);
      }
      cairo_destroy(cr);
  ...
  for (int y = 0; y < *height; ++y) {
    for (int x = 0; x < *width; ++x) {
      ...
      out_row[x] = ((r << RrDefaultRedOffset) | (g << RrDefaultGreenOffset) | (b << RrDefaultBlueOffset) |
