...
void ping_start(struct _ObClient* client, ObPingEventHandler h) {
  ...
  if (g_hash_table_find(ping_ids, find_client, client) != NULL)
    return;
  ...
  t->loopid = g_timeout_add_full(G_PRIORITY_DEFAULT, PING_TIMEOUT, ping_timeout, t, NULL);
...
static void ping_send(ObPingTarget* t) {
  ...
  if (t->id == 0 || t->waiting > 0) {
    ...
    t->id = ping_next_id;
    if (++ping_next_id == 0)
      ++ping_next_id; /* skip 0 on wraparound */
  ...
  OBT_PROP_MSG_TO(t->client->window, t->client->window, WM_PROTOCOLS, OBT_PROP_ATOM(NET_WM_PING), t->id,
                  t->client->window, 0, 0, NoEventMask);
}
