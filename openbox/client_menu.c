...
enum { LAYER_TOP = 1, LAYER_NORMAL = 0, LAYER_BOTTOM = -1 };
...
static void set_icon_color(ObMenuEntry* e) {
  ...
  e->data.normal.mask_disabled_selected_color = ob_rr_theme->menu_disabled_selected_color;
}
    ...
    if (e->type == OB_MENU_ENTRY_TYPE_NORMAL) {
      switch (e->id) {
        ...
        case CLIENT_MAXIMIZE:
          *en = ((c->functions & OB_CLIENT_FUNC_MAXIMIZE) && (!c->max_horz || !c->max_vert));
          break;
        case CLIENT_SHADE:
...
static void client_menu_execute(ObMenuEntry* e, ObMenuFrame* f, ObClient* c, guint state, gpointer data) {
  gint x, y;
  gulong ignore_start;
  ...
  switch (e->id) {
    ...
    case CLIENT_MOVE:
      ...
      moveresize_start(c, x, y, 0, OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE_KEYBOARD));
      break;
    case CLIENT_RESIZE:
      /* this needs to grab the keyboard so hide the menu */
      menu_frame_hide_all();
      ...
      screen_pointer_pos(&x, &y);
      moveresize_start(c, x, y, 0, OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_KEYBOARD));
      break;
    case CLIENT_CLOSE:
      client_close(c);
      break;
...
static void layer_menu_execute(ObMenuEntry* e, ObMenuFrame* f, ObClient* c, guint state, gpointer data) {
  gulong ignore_start;
...
static gboolean send_to_menu_update(ObMenuFrame* frame, gpointer data) {
  ...
  for (it = menu->entries; it; it = g_list_next(it)) {
    ...
    if ((desk == DESKTOP_ALL && c->desktop != DESKTOP_ALL) || (c->desktop == DESKTOP_ALL && desk == screen_desktop)) {
...
static void send_to_menu_execute(ObMenuEntry* e, ObMenuFrame* f, ObClient* c, guint state, gpointer data) {
  g_assert(c);
...
static void client_menu_place(ObMenuFrame* frame, gint* x, gint* y, gboolean mouse, gpointer data) {
  gint dx, dy;
  ...
  if (!mouse && frame->client) {
    ...
    *y = frame->client->frame->area.y + frame->client->frame->size.top - frame->client->frame->bwidth;
    menu_frame_move_on_screen(frame, *x, *y, &dx, &dy);
    if (dy != 0) {
      /* try above the titlebar */
      *y = frame->client->frame->area.y + frame->client->frame->bwidth - frame->area.height;
      menu_frame_move_on_screen(frame, *x, *y, &dx, &dy);
    }
    if (dy != 0) {
