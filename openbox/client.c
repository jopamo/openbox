...
static ObAppSettings* client_get_settings_state(ObClient* self) {
  ...
  for (it = config_per_app_settings; it; it = g_slist_next(it)) {
    ...
    if (app->name && !g_pattern_spec_match(app->name, strlen(self->name), self->name, NULL)) {
      match = FALSE;
    }
    else if (app->group_name &&
             !g_pattern_spec_match(app->group_name, strlen(self->group_name), self->group_name, NULL)) {
      match = FALSE;
    }
    else if (app->class && !g_pattern_spec_match(app->class, strlen(self->class), self->class, NULL)) {
      match = FALSE;
    }
    else if (app->group_class &&
             !g_pattern_spec_match(app->group_class, strlen(self->group_class), self->group_class, NULL)) {
      match = FALSE;
    }
    else if (app->role && !g_pattern_spec_match(app->role, strlen(self->role), self->role, NULL)) {
      match = FALSE;
    }
    else if (app->title && self->title && !g_pattern_spec_match(app->title, strlen(self->title), self->title, NULL)) {
      match = FALSE;
    }
...
void client_setup_decor_and_functions(ObClient* self, gboolean reconfig) {
  ...
  if (self->fullscreen) {
    self->functions &=
        (OB_CLIENT_FUNC_CLOSE | OB_CLIENT_FUNC_MOVE | OB_CLIENT_FUNC_FULLSCREEN | OB_CLIENT_FUNC_ICONIFY);
    self->decorations = 0;
  }
