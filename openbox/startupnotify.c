...
void sn_startup(gboolean reconfig) {
  ...
  sn_context = sn_monitor_context_new(sn_display, ob_screen, sn_event_func, NULL, NULL);
  ...
  switch (sn_monitor_event_get_type(ev)) {
    case SN_MONITOR_EVENT_INITIATED:
      ...
      /* 20 second timeout for apps to start if the launcher doesn't
         have a timeout */
      g_timeout_add_full(G_PRIORITY_DEFAULT, 20 * 1000, sn_wait_timeout, seq,
...
Time sn_app_started(const gchar* id, const gchar* wmclass, const gchar* name) {
  ...
  for (it = sn_waits; it; it = g_slist_next(it)) {
    ...
    if (id && seqid) {
      ...
      */
      found = (seqclass && !strcmp(seqclass, wmclass)) || (seqclass && !strcmp(seqclass, name));
    }
    else if (seqbin) {
      /* Check the binary name against the class and name hints
         as well, to help apps that don't have the class set
         correctly */
      found = (seqbin && !g_ascii_strcasecmp(seqbin, wmclass)) || (seqbin && !g_ascii_strcasecmp(seqbin, name));
    }
