...
static gboolean resist_move_window(Rect window, Rect target, gint resist, gint* x, gint* y) {
  ...
  if (!snapx) {
    if (ct < tb && cb > tt) {
      if (cl >= tr && l < tr && l >= tr - resist)
        *x = tr, snapx = TRUE;
      else if (cr <= tl && r > tl && r <= tl + resist)
        *x = tl - w + 1, snapx = TRUE;
      if (snapx) {
        /* try to corner snap to the window */
        if (ct > tt && t <= tt && t > tt - resist)
          *y = tt + 1, snapy = TRUE;
        else if (cb < tb && b >= tb && b < tb + resist)
          *y = tb - h, snapy = TRUE;
      }
    }
  }
  if (!snapy) {
    if (cl < tr && cr > tl) {
      if (ct >= tb && t < tb && t >= tb - resist)
        *y = tb, snapy = TRUE;
      else if (cb <= tt && b > tt && b <= tt + resist)
        *y = tt - h + 1, snapy = TRUE;
      if (snapy) {
        /* try to corner snap to the window */
        if (cl > tl && l <= tl && l > tl - resist)
          *x = tl + 1, snapx = TRUE;
        else if (cr < tr && r >= tr && r < tr + resist)
          *x = tr - w, snapx = TRUE;
      }
    }
  }
...
void resist_move_windows(ObClient* c, gint resist, gint* x, gint* y) {
  ...
  for (it = stacking_list; it; it = g_list_next(it)) {
    ...
    if (resist_move_window(c->frame->area, target->frame->area, resist, x, y))
      break;
  }
...
void resist_move_monitors(ObClient* c, gint resist, gint* x, gint* y) {
  ...
  RECT_SET(desired_area, c->frame->area.x, c->frame->area.y, c->frame->area.width, c->frame->area.height);
  ...
  for (i = 0; i < screen_num_monitors; ++i) {
    ...
    area = screen_area(c->desktop, SCREEN_AREA_ALL_MONITORS, &desired_area);
...
static gboolean resist_size_window(Rect window, Rect target, gint resist, gint* w, gint* h, ObDirection dir) {
...
void resist_size_windows(ObClient* c, gint resist, gint* w, gint* h, ObDirection dir) {
  GList* it;
  ...
  for (it = stacking_list; it; it = g_list_next(it)) {
    ...
    if (resist_size_window(c->frame->area, target->frame->area, resist, w, h, dir))
      break;
  }
  dock_get_area(&dock_area);
  resist_size_window(c->frame->area, dock_area, resist, w, h, dir);
}

void resist_size_monitors(ObClient* c, gint resist, gint* w, gint* h, ObDirection dir) {
  gint l, t, r, b; /* my left, top, right and bottom sides */
  gint dlt, drb;   /* my destination left/top and right/bottom sides */
  Rect* area;
  const Rect* parea;
